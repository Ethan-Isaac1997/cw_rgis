...
# Reference: Chapter 2
df_site <- df_fish %>% distinct(site_id, lon, lat) %>% st_as_sf(coords = c("lon", "lat"), crs = 4326)
...
df_site_w_county <- sf_site_w_county %>% as_tibble()
...
ggplot() +
  geom_sf(data = sf_county_s1, fill = "lightgray", color = "black") +  
  geom_sf(data = sf_site, color = "blue", size = 2) +                   
  theme_minimal() +
  labs(title = "Sampling Sites and Counties with at Least One Site",
...
# Transform to a projected CRS (UTM Zone 17N) for accurate area calculation
sf_county_s1_proj<-st_transform(sf_county_s1, crs = 32617)

sf_county_area<-sf_county_s1_proj %>%
  arrange(desc(st_area(geometry))) %>%
  slice_head(n = 1)

##County Name - Bladen

##confirm area matches
sf_county_area %>% 
  st_area()
